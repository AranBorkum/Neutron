...
int main(int argc, char** argv) {
  ...
  // im->SetInputTree("FullGeoAna"); 
  im->SetInputTree("fullgeoana/FullGeoAna");
  im->LoadTree();
  ...
  TH2D     *nCaptureYZ      = new TH2D    ("nCaptureYZ" , "nCaptureYZ" , 2000, -1000,  7000, 2000, -1300, 3000);
  TH2D     *nStartXY        = new TH2D    ("nStartXY"   , "nStartXY"   ,  500, -1300,  1300,  500, -1500, 3000);
  TH2D     *nStartYZ        = new TH2D    ("nStartYZ"   , "nStartYZ"   , 2000, -1000,  7000, 2000, -1300, 3000);
  ...
  for (int CurrentEvent=0; CurrentEvent<fNEvent; ++CurrentEvent) {
	...
	if (CurrentMaterial == itt) inVector = true;
      if (!inVector) materials.push_back(CurrentMaterial);
    }
  }
  std::cout << std::endl;
