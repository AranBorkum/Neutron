...
double GenVolume(int simType) {
  // --- Get the legths of each slab
  double TotalVolume = 0;
  if (simType == 0) {
    double xSpan = abs(structureDimX[0]) + abs(structureDimX[1]);
    double ySpan = abs(structureDimY[0]) + abs(structureDimY[1]);
    double zSpan = abs(structureDimZ[0]) + abs(structureDimZ[1]);
    
    // --- Get the volume of each slab
    double xVolume = ySpan * zSpan;
    double yVolume = xSpan * zSpan;
    double zVolume = xSpan * ySpan;
    
    // --- Get the total volume
    TotalVolume = (2.0*xVolume) + (2.0*yVolume) + (2.0*zVolume);
  }
  ...
  if (simType == 2) {
    ...
    TotalVolume = midSlab + (2.0*tbSlab) +(2.0*fbSlab);
  }
...
int ValidationAna(ParsedXML xml, int nEvent, int SimType) {
  ...
  double RockMass         = GenVolume(SimType) * 2.82;
  double RatePerGram      = ProdRateYear / RockMass;
  double RatePerCCPerYear = ProdRateYear / GenVolume(SimType);
  ...
  file << "Neutron production rate:           " << RatePerGram << " [n/y/g]"	<< std::endl;
