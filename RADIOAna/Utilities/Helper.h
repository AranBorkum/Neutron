...
void progressTable(std::vector<ParsedXML> xmls, int i) {
  ...
  for (int it=0; it<xmls.size(); ++it) {
    ...
    if      (it <  i  )                     { std::cout << " DONE \u2713  \r"; }
    else if (it == i && i != xmls.size()-1) { std::cout << " Running\r"      ; }
    else if (i == xmls.size()-1)            { std::cout << " DONE \u2713  \r"; }
    else                                    { std::cout << "\r";               }
  }
